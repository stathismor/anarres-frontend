    ...
    <b-card no-body>
      ...
      <div class="card-actions">
        <b-button variant="outline-danger" @click="doClear()">
          ...
          <translate key="lang_btn_clear_requests"
            >Clear Pending Requests</translate
          >
        </b-button>
      </div>
      <data-table
  ...
  data() {
    return {
      fields: [
        {
          key: 'timestamp',
          label: this.$gettext('Date Requested'),
          sortable: false,
        },
        {
          key: 'played_at',
          label: this.$gettext('Date Played'),
          sortable: false,
        },
        {
          key: 'song_title',
  ...
  methods: {
    ...
    doClear() {
      this.$confirmDelete({
        ...
        if (result.value) {
          this.$wrapWithLoading(this.axios.post(this.clearUrl)).then((resp) => {
            this.$notifySuccess(resp.data.message);
            this.$refs.datatable.refresh();
          });
        }
