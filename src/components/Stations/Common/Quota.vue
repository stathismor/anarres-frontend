...
export default {
  ...
  computed: {
    ...
    langSpaceUsed() {
      ...
      if (null !== this.quota.num_files) {
        const langNumFiles = this.$ngettext(
          '%{filesCount} File',
          '%{filesCount} Files',
          this.quota.num_files
        );
        const langNumFilesParsed = this.$gettextInterpolate(langNumFiles, {
          filesCount: this.quota.num_files,
        });
  ...
  methods: {
    update() {
      this.axios.get(this.quotaUrl).then((resp) => {
