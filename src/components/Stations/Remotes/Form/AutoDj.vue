  ...
  <b-tab :title="langTabTitle">
    <b-form-row class="mb-3">
      ...
      >
        ...
        <template #description="{ lang }">
          <translate :key="lang"
            >If enabled, the AutoDJ on this installation will automatically play
            music to this mount point.</translate
          >
        </template>
      </b-wrapped-form-checkbox>
    </b-form-row>
    ...
    <b-form-row v-if="form.enable_autodj.$model">
      ...
      >
        ...
        <template #description="{ lang }">
          <translate :key="lang"
            >If the port you broadcast to is different from the one you listed
            in the URL above, specify the source port here.</translate
          >
        </template>
      </b-wrapped-form-group>
      ...
      >
        <template #label="{ lang }">
          <translate :key="lang"
            >Remote Station Source Mountpoint/SID</translate
          >
        </template>
        <template #description="{ lang }">
          <translate :key="lang"
            >If the mountpoint (i.e. <code>/radio.mp3</code>) or Shoutcast SID
            (i.e. <code>2</code>) you broadcast to is different from the one
            listed above, specify the source mount point here.</translate
          >
        </template>
      </b-wrapped-form-group>
      ...
      >
        ...
        <template #description="{ lang }">
          <translate :key="lang"
            >If you are broadcasting using AutoDJ, enter the source username
            here. This may be blank.</translate
          >
        </template>
      </b-wrapped-form-group>
      ...
      >
        ...
        <template #description="{ lang }">
          <translate :key="lang"
            >If you are broadcasting using AutoDJ, enter the source password
            here.</translate
          >
        </template>
      </b-wrapped-form-group>
      ...
      >
        ...
        <template #description="{ lang }">
          <translate :key="lang"
            >Enable to advertise this relay on "Yellow Pages" public radio
