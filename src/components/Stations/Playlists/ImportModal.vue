  ...
  <b-modal id="import_modal" ref="modal" :title="langTitle">
    <b-form class="form" @submit.prevent="doSubmit">
      <b-form-group label-for="import_modal_playlist_file">
        <template #label>
          <translate key="lang_form_playlist_file"
            >Select PLS/M3U File to Import</translate
          >
        </template>
        <template #description>
          <translate key="lang_form_playlist_file_desc"
            >AzuraCast will scan the uploaded file for matches in this station's
            music library. Media should already be uploaded before running this
            step. You can re-run this tool as many times as needed.</translate
          >
        </template>
        <b-form-file
...
export default {
  ...
  methods: {
    ...
    doSubmit() {
      ...
      this.$wrapWithLoading(this.axios.post(this.importPlaylistUrl, formData))
        .then((resp) => {
