<template>
  <b-tab :title="langTabTitle">
    <b-form-group v-if="scheduleItems.length === 0">
      <label>
        <translate key="lang_playlist_schedule_not_scheduled"
          >Not Scheduled</translate
        >
      </label>
      <p>
        <translate key="lang_playlist_schedule_desc"
          >This playlist currently has no scheduled times. It will play at all
          times. To add a new scheduled time, click the button below.</translate
        >
      </p>
    </b-form-group>

    <b-card
      v-for="(row, index) in form.schedule_items.$each.$iter"
      :key="index"
      class="mb-3"
      no-body
    >
      <div class="card-header bg-primary-dark d-flex align-items-center">
        <div class="flex-fill">
          <h2 class="card-title">
            <translate
              :key="'lang_schedule_entry_' + index"
              :translate-params="{ num: parseInt(index) + 1 }"
              >Scheduled Time #%{num}</translate
            >
          </h2>
        </div>
          ...
          <b-form-row>
            ...
            >
              ...
              <template #description="{ lang }">
                <translate :key="lang"
                  >To play once per day, set the start and end times to the same
                  value.</translate
                >
              </template>
            ...
            >
              ...
              <template #description="{ lang }">
                <translate :key="lang"
                  >If the end time is before the start time, the playlist will
                  play overnight.</translate
                >
              </template>
              <template #default="props">
                <playlist-time
            ...
            <b-col md="4" class="form-group">
              ...
              <div>
                <translate
                  key="lang_station_tz_desc"
                  :translate-params="{ tz: stationTimeZone }"
                  >This station's time zone is currently %{tz}.</translate
                >
              </div>
            </b-col>
            ...
            >
              ...
              <template #description="{ lang }">
                <translate :key="lang"
                  >To set this schedule to run only within a certain date range,
                  specify a start and end date.</translate
                >
                <div v-if="hasAdvancedSettings" class="text-danger">
                  <translate :key="lang + '_warning'"
                    >Start/end date cannot be used on playlists with advanced
                    settings!</translate
                  >
                </div>
              </template>
            ...
            >
              ...
              <template #description="{ lang }">
                <translate :key="lang"
                  >Only loop through playlist once.</translate
                >
              </template>
            </b-wrapped-form-checkbox>
            ...
            >
              ...
              <template #description="{ lang }">
                <translate :key="lang"
                  >Leave blank to play on every day of the week.</translate
                >
              </template>
              <template #default="props">
                <b-checkbox-group
