<template>
  ...
  >
    <p>
      <translate key="queue_info"
        >This queue contains the remaining tracks in the order they will be
        queued by the AzuraCast AutoDJ (if the tracks are eligible to be
        played).</translate
      >
    </p>
...
export default {
  ...
  methods: {
    ...
    doClear() {
      this.$wrapWithLoading(this.axios.delete(this.queueUrl)).then((resp) => {
