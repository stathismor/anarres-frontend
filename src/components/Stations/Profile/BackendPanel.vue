    ...
    <div class="card-header bg-primary-dark">
      <h3 class="card-title">
        ...
        <small
          ...
          v-if="np.services.backend_running"
          key="lang_profile_backend_running"
          v-translate
          >Running</small
        >
        <small
          ...
          v-else
          key="lang_profile_backend_not_running"
          v-translate
          >Not Running</small
        >
        <br />
    ...
    <div class="card-body">
      ...
      <div class="buttons" v-if="userCanManageMedia">
        <a
          ...
          :href="manageMediaUri"
          key="lang_profile_manage_media"
          >Music Files</a
        >
        <a
          class="btn btn-primary"
          :href="managePlaylistsUri"
          key="lang_profile_manage_playlists"
          v-translate
          >Playlists</a
        >
      </div>
...
export default {
  ...
  computed: {
    langTotalTracks() {
      let numSongsRaw = this.$ngettext(
        '%{numSongs} uploaded song',
        '%{numSongs} uploaded songs',
        this.numSongs
      );
      let numSongs = this.$gettextInterpolate(numSongsRaw, {
        numSongs: this.numSongs,
      });

      let numPlaylistsRaw = this.$ngettext(
        '%{numPlaylists} playlist',
        '%{numPlaylists} playlists',
        this.numPlaylists
      );
      let numPlaylists = this.$gettextInterpolate(numPlaylistsRaw, {
        numPlaylists: this.numPlaylists,
      });
