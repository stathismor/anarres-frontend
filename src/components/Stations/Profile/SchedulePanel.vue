  ...
  <div id="profile-scheduled">
    ...
    >
      <div class="card-header bg-primary-dark">
        <h3 class="card-title" key="lang_schedule_title" v-translate>
          Scheduled
        </h3>
      </div>
            ...
            <td>
              ...
              >
                <h5 class="m-0">
                  <small>
                    <template v-if="row.type === 'playlist'">
                      <translate key="lang_schedule_playlist_name"
                        >Playlist</translate
                      >
                    </template>
                    <template v-else>
                      <translate key="lang_schedule_streamer_name"
                        >Streamer/DJ</translate
                      >
                    </template> </small
                  ><br />
                  {{ row.name }}
                ...
                <p class="text-right m-0">
                  <small
                    >{{ row.start_formatted }} - {{ row.end_formatted }}</small
                  >
                  <br />
                  <strong>
  ...
  computed: {
    processedScheduleItems() {
      ...
      return _.map(this.scheduleItems, (row) => {
        const start_moment = DateTime.fromSeconds(row.start_timestamp).setZone(
          this.stationTimeZone
        );
        const end_moment = DateTime.fromSeconds(row.end_timestamp).setZone(
          this.stationTimeZone
        );
