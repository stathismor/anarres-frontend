    ...
    <div class="col-12">
      <div class="card">
        ...
        >
          ...
          <div class="flex-fill">
            <p class="mb-0">
              <translate key="lang_sftp_details"
                >You can also upload files in bulk via SFTP.</translate
              >
            </p>
          </div>
        ...
        >
          <template #cell(path)="row">
            <div
              :class="{ is_dir: row.item.is_dir, is_file: !row.item.is_dir }"
            >
              <album-art
              ...
              <template v-else-if="row.item.media_is_playable">
                <a
                  class="name"
                  :href="row.item.media_links_play"
                  target="_blank"
                  :title="row.item.name"
                >
                  {{ row.item.text }}
              ...
              <br />
              <small v-if="row.item.media_is_playable">{{
                row.item.path_short
              }}</small>
          ...
          <template #cell(commands)="row">
            <template v-if="row.item.media_links_edit">
              <b-button
                ...
                @click.prevent="
                  edit(
                    row.item.media_links_edit,
                    row.item.media_links_art,
                    row.item.media_links_play,
                    row.item.media_links_waveform
                  )
                "
              >
                {{ langEditButton }}
...
export default {
  ...
  methods: {
    ...
    onHashChange() {
      ...
      let urlHash = decodeURIComponent(
        window.location.hash.substr(1).replace(/\+/g, '%20')
      );

      if ('' !== urlHash) {
        if (
          urlHash.substr(0, 9) === 'playlist:' ||
          urlHash.substr(0, 8) === 'special:'
        ) {
