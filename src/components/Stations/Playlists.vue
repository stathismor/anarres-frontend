    ...
    <b-card no-body>
      <b-card-header header-bg-variant="primary-dark">
        <b-row class="align-items-center">
          <b-col md="6">
            <h2 class="card-title" key="lang_playlists" v-translate>
              Playlists
            </h2>
          </b-col>
          ...
          >
            <template #cell(actions)="row">
              <b-button-group size="sm">
                ...
                >
                  <b-dropdown-item
                    @click.prevent="doModify(row.item.links.toggle)"
                  >
                  ...
                  <b-dropdown-item
                    ...
                    v-if="
                      row.item.source === 'songs' &&
                      row.item.order === 'sequential'
                    "
                  >
                    {{ langReorderButton }}
                  </b-dropdown-item>
                  <b-dropdown-item
                    ...
                    v-if="
                      row.item.source === 'songs' && row.item.order !== 'random'
                    "
                  >
                    {{ langQueueButton }}
                  </b-dropdown-item>
                  ...
                  <b-dropdown-item
                    @click.prevent="
                      doClone(row.item.name, row.item.links.clone)
                    "
                  >
                    {{ langCloneButton }}
                  </b-dropdown-item>
                  <template v-for="format in ['pls', 'm3u']">
                    ...
                    >
                      <translate
                        :key="'lang_format_' + format"
                        :translate-params="{ format: format.toUpperCase() }"
                      >
                        Export %{format}
                      </translate>
                    </b-dropdown-item>
            ...
            <template #cell(num_songs)="row">
              <template v-if="row.item.source === 'songs'">
                <a
                  :href="
                    filesUrl + '#playlist:' + encodeURIComponent(row.item.name)
                  "
                >
                  {{ row.item.num_songs }}
                </a>
...
export default {
      ...
      fields: [
        {
          key: 'name',
          isRowHeader: true,
          label: this.$gettext('Playlist'),
          sortable: true,
        },
        {
          key: 'scheduling',
          label: this.$gettext('Scheduling'),
          sortable: false,
        },
  ...
  methods: {
    ...
    doModify(url) {
      this.$wrapWithLoading(this.axios.put(url)).then((resp) => {
        this.needsRestart();
    ...
    doDelete(url) {
      this.$confirmDelete({
        ...
        if (result.value) {
          this.$wrapWithLoading(this.axios.delete(url)).then((resp) => {
            this.needsRestart();
