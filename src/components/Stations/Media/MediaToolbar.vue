            ...
            <div v-for="playlist in playlists" class="form-group">
              <div class="custom-control custom-checkbox">
                <input
                  ...
                  v-bind:id="'chk_playlist_' + playlist.id"
                  name="playlists[]"
                ...
                <label
                  ...
                  v-bind:for="'chk_playlist_' + playlist.id"
                >
                  {{ playlist.name }}
                </label>
              </div>
...
export default {
  ...
  methods: {
    ...
    doReprocess(e) {
      this.doBatch(
        'reprocess',
        this.$gettext('Files marked for reprocessing:')
      );
    },
    doDelete(e) {
        ...
        this.$wrapWithLoading(
          ...
          if (resp.data.success) {
            ...
            _.forEach(this.selectedItems.all, (item) => {
              allItemNodes.push(
                this.$createElement('div', {}, item.path_short)
              );
            });
      ...
      if (this.selectedItems.all.length) {
        this.$wrapWithLoading(
          ...
          if (resp.data.success) {
            ...
            let notifyMessage =
              this.checkedPlaylists.length > 0
