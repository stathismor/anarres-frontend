<template>
  <b-tab :title="langTitle" lazy>
    <p>
      <translate key="lang_waveform_editor_desc"
        >Set cue and fade points using the visual editor. The timestamps will be
        saved to the corresponding fields in the advanced playback
        settings.</translate
      >
    </p>
    ...
    <b-form-group>
      <b-button-group>
        <b-button variant="light" @click="playAudio">
          ...
          <span class="sr-only"
            ><translate key="lang_btn_play">Play</translate></span
          >
        </b-button>
        <b-button variant="dark" @click="stopAudio">
          <icon icon="stop"></icon>
          <span class="sr-only"
            ><translate key="lang_btn_stop">Stop</translate></span
          >
        </b-button>
      </b-button-group>
      <b-button-group>
  ...
  methods: {
    ...
    updateRegions() {
      ...
      if (fade_overlap > cue_in) {
        this.$refs.waveform.addRegion(
          cue_out - fade_overlap,
          cue_out,
          'hsla(29,100%,48%,0.4)'
        );
      }
