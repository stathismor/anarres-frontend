      ...
      <b-form-row class="mb-3">
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >This name should always begin with a slash (/), and must be a
              valid URL, such as /autodj.mp3</translate
            >
          </template>
        </b-wrapped-form-group>
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >The display name assigned to this mount point when viewing it on
              administrative or public pages. Leave blank to automatically
              generate one.</translate
            >
          </template>
        </b-wrapped-form-group>
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >Enable to allow listeners to select this mount point on this
              station's public pages.</translate
            >
          </template>
        </b-wrapped-form-checkbox>
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >If this mount is the default, it will be played on the radio
              preview and the public radio page in this system.</translate
            >
          </template>
        </b-wrapped-form-checkbox>
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >Enter the full URL of another stream to relay its broadcast
              through this mount point.</translate
            >
          </template>
        </b-wrapped-form-group>
        ...
        >
          <template #label="{ lang }">
            <translate :key="lang"
              >Publish to "Yellow Pages" Directories</translate
            >
          </template>
          <template #description="{ lang }">
            <translate :key="lang"
              >Enable to advertise this mount point on "Yellow Pages" public
              radio directories.</translate
            >
          </template>
        </b-wrapped-form-checkbox>
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >Set the length of time (seconds) a listener will stay connected
              to the stream. If set to 0, listeners can stay connected
              infinitely.</translate
            >
          </template>
        </b-wrapped-form-group>
        ...
        <template v-if="isShoutcast">
          ...
          >
            ...
            <template #description
              ><span v-html="langAuthhashDesc"></span
            ></template>
          </b-wrapped-form-group>
        </template>
        <template v-if="isIcecast">
          ...
          >
            ...
            <template #description="{ lang }">
              <translate :key="lang"
                >If this mount point is not playing audio, listeners will
                automatically be redirected to this mount point. The default is
                /error.mp3, a repeating error message.</translate
              >
            </template>
          </b-wrapped-form-group>
        </template>
