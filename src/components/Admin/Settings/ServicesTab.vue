<template>
  <b-tab :title="langTabTitle" :title-link-class="tabClass">
    <b-form-fieldset>
      <template #label>
        <translate key="lang_section_update_checks"
          >AzuraCast Update Checks</translate
        >
      </template>
      ...
      <b-form-row>
        <b-form-markup class="col-md-6" id="form_release_channel">
          ...
          <template #description="{ lang }">
            ...
            >
              <translate :key="lang"
                >Learn more about release channels in the AzuraCast
                docs.</translate
              >
            </a>
          </template>
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >Show new releases within your update channel on the AzuraCast
              homepage.</translate
            >
          </template>
        </b-wrapped-form-checkbox>
    ...
    <b-form-fieldset>
      <template #label>
        <translate key="lang_section_email_delivery"
          >E-mail Delivery Service</translate
        >
      </template>
      <template #description>
        <translate key="lang_section_email_delivery_desc"
          >Used for "Forgot Password" functionality, web hooks and other
          functions.</translate
        >
      </template>
      ...
      <b-form-row v-if="form.mail_enabled.$model">
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >Usually enabled for port 465, disabled for ports 587 or
              25.</translate
            >
          </template>
        </b-wrapped-form-checkbox>
    ...
    <b-form-fieldset>
      ...
      <b-form-row>
        ...
        >
          <template #label="{ lang }">
            <translate :key="lang"
              >Check Web Services for Album Art for "Now Playing"
              Tracks</translate
            >
          </template>
        </b-wrapped-form-checkbox>
        ...
        >
          <template #label="{ lang }">
            <translate :key="lang"
              >Check Web Services for Album Art When Uploading Media</translate
            >
          </template>
        </b-wrapped-form-checkbox>
        ...
        >
          ...
          <template #description="{ lang }">
            <translate :key="lang"
              >This service can provide album art for tracks where none is
              available locally.</translate
            >
            <br />
            <a href="https://www.last.fm/api/account/create" target="_blank">
              <translate :key="lang + '2'"
                >Apply for an API key at Last.fm</translate
              >
            </a>
          </template>
        </b-wrapped-form-group>
