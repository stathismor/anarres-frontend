<template>
  <b-tab :title="langTabTitle">
    <b-card
      v-for="(row, index) in form.permissions.station.$each.$iter"
      :key="index"
      class="mb-3"
      no-body
    >
          ...
          <b-form-row>
            ...
            >
              ...
              <template #description="{ lang }">
                <translate :key="lang"
                  >Users with this role will have these permissions for this
                  single station.</translate
                >
              </template>
  ...
  computed: {
    ...
    remainingStations() {
      return _.pickBy(this.stations, (stationName, stationId) => {
        return !_.find(this.form.permissions.station.$model, {
