    ...
    <b-form class="form vue-form" @submit.prevent="submit">
      <b-tabs :card="!isModal" lazy justified :content-class="tabContentClass">
        <admin-stations-profile-form
          :form="$v.form"
          :tab-class="getTabClass($v.profileTab)"
          :timezones="timezones"
          :show-advanced="showAdvanced"
        ></admin-stations-profile-form>
        <admin-stations-frontend-form
          :form="$v.form"
          :tab-class="getTabClass($v.frontendTab)"
        ...
        <admin-stations-backend-form
          :form="$v.form"
          :tab-class="getTabClass($v.backendTab)"
          :show-advanced="showAdvanced"
        ></admin-stations-backend-form>
        <admin-stations-admin-form
          v-if="showAdminTab"
          :tab-class="getTabClass($v.adminTab)"
          :form="$v.form"
...
export default {
  ...
  mixins: [validationMixin, StationFormProps],
  validations() {
    let formValidations = {
      form: {
        name: { required },
      ...
      profileTab: [
        ...
        'form.url',
        'form.timezone',
        'form.enable_public_page',
        'form.enable_on_demand',
        'form.enable_on_demand_download',
        'form.default_album_art_url',
      ],
      frontendTab: ['form.frontend_type', 'form.frontend_config'],
      backendTab: [
        'form.backend_type',
        'form.backend_config',
    ...
    if (this.showAdvanced) {
      const advancedValidations = {
        ...
        profileTab: ['form.short_name', 'form.api_history_items'],
      };
    ...
    if (this.showAdminTab) {
      const adminValidations = {
        ...
        adminTab: [
          'form.media_storage_location',
          'form.recordings_storage_location',
          'form.podcasts_storage_location',
          'form.is_enabled',
        ],
      };
  ...
  methods: {
    ...
    doLoad() {
      this.$wrapWithLoading(this.axios.get(this.editUrl))
        .then((resp) => {
    ...
    submit() {
      ...
      this.$wrapWithLoading(
        this.axios({
          ...
          url: this.isEditMode ? this.editUrl : this.createUrl,
