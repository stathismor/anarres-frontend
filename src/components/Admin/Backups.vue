  ...
  <div>
    ...
    <div class="card-deck">
      <section class="card mb-3" role="region">
        ...
        <div class="card-actions">
          ...
          >
            ...
            <translate key="lang_btn_last_backup"
              >Most Recent Backup Log</translate
            >
          </b-button>
        </div>
      </section>
      ...
      <section class="card mb-3" role="region">
        <b-card-header header-bg-variant="primary-dark">
          <h2 class="card-title">
            <translate key="lang_hdr_restoring_backups"
              >Restoring Backups</translate
            >
          </h2>
        </b-card-header>
        ...
        <div class="card-body">
          <p class="card-text">
            <translate key="lang_restore_1"
              >To restore a backup from your host computer, run:</translate
            >
          </p>

          <pre
            v-if="isDocker"
          ><code>./docker.sh restore path_to_backup.zip</code></pre>
          <pre
            v-else
          ><code>/var/azuracast/www/bin/console azuracast:restore path_to_backup.zip</code></pre>

          <p class="card-text text-warning">
            <translate key="lang_restore_2"
              >Note that restoring a backup will clear your existing database.
              Never restore backup files from untrusted users.</translate
            >
          </p>
        </div>
      </section>
...
export default {
  ...
  methods: {
    relist() {
      ...
      this.$wrapWithLoading(this.axios.get(this.settingsUrl)).then((resp) => {
        this.settings = {
