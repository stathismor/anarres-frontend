  ...
  >
    <b-row>
      <b-col md="7">
        ...
        <p class="card-text">
          <translate key="lang_2fa_1"
            >From your smartphone, scan the code to the right using an
            authentication app of your choice (FreeOTP, Authy, etc).</translate
          >
        </p>

        <h5 class="mt-0">
          <translate key="lang_2fa_hdr_2"
            >Step 2: Verify Generated Code</translate
          >
        </h5>

        <p class="card-text">
          <translate key="lang_2fa_2"
            >To verify that the code was set up correctly, enter the 6-digit
            code the app shows you.</translate
          >
        </p>
        ...
        <b-form-fieldset>
          <b-wrapped-form-group id="form_otp" :field="$v.form.otp" autofocus>
            ...
            <template #description="{ lang }">
              <translate :key="lang"
                >Enter the current code provided by your authenticator app to
                verify that it's working correctly.</translate
              >
            </template>
...
export default {
  ...
  methods: {
    ...
    open() {
      ...
      this.$wrapWithLoading(this.axios.put(this.twoFactorUrl))
        .then((resp) => {
          this.totp = resp.data;
          this.loading = false;
        })
