<template>
  <div id="dashboard">
    <section class="card mb-4" role="region">
      <div
        class="card-header bg-primary-dark d-flex flex-wrap align-items-center"
      >
        <avatar
        ...
        <div class="flex-md-shrink-0 mt-3 mt-md-0">
          ...
          <a
            v-if="showAdmin"
            class="btn btn-bg ml-2"
            role="button"
            :href="adminUrl"
          >
            <icon icon="settings"></icon>
            <translate key="dashboard_btn_administration"
    ...
    <section class="card mb-4" role="region" v-if="showCharts">
      <div class="card-header bg-primary-dark d-flex align-items-center">
        <div class="flex-fill">
          <h3 class="card-title">
            <translate key="dashboard_header_listeners_per_station"
              >Listeners Per Station</translate
            >
          </h3>
        </div>
    ...
    <section class="card" role="region">
      ...
      >
        <div class="flex-fill">
          <h2 class="card-title">
            <translate key="dashboard_header_stations"
              >Station Overview</translate
            >
          </h2>
        </div>
        <div class="flex-shrink-0" v-if="showAdmin">
          ...
          >
            <icon icon="settings"></icon>
            <translate key="dashboard_btn_manage_stations"
              >Manage Stations</translate
            >
          </b-button>
        </div>
      </div>
      ...
      <b-overlay variant="card" :show="stationsLoading">
        <div class="card-body py-3" v-if="stationsLoading">&nbsp;</div>
        <table
        ...
        >
          ...
          <tbody>
            ...
            >
              ...
              <td class="pl-2">
                <big>{{ item.station.name }}</big
                ><br />
                <a :href="item.links.public" target="_blank">
                  <translate key="dashboard_link_public_page"
                    >Public Page</translate
                  >
                </a>
              </td>
              <td class="text-center">
                <span class="nowplaying-listeners">{{
              ...
              <td>
                <div class="d-flex align-items-center">
                  ...
                  <div
                    v-if="item.now_playing.song.title !== ''"
                    class="flex-fill"
                  >
                    <strong
                      ><span class="nowplaying-title">
                        {{ item.now_playing.song.title }}
                      </span></strong
                    ><br />
                    <span class="nowplaying-artist">{{
              ...
              <td class="text-right">
                <a class="btn btn-primary" v-bind:href="item.links.manage">
                  <translate key="dashboard_btn_manage_station"
                    >Manage</translate
                  >
                </a>
              </td>
