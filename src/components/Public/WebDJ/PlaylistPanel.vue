    ...
    <div class="card-body">
      ...
      <div class="mt-3" v-if="playing">
        ...
        <div class="progress mb-1">
          <div
            class="progress-bar"
            v-bind:style="{ width: volumeLeft + '%' }"
          ></div>
        </div>
        <div class="progress">
          <div
            class="progress-bar"
            v-bind:style="{ width: volumeRight + '%' }"
          ></div>
        </div>
      </div>
      ...
      <div class="form-group mt-2">
        <div class="custom-file">
          <input
            ...
            accept="audio/*"
            multiple="multiple"
            v-on:change="addNewFiles($event.target.files)"
          />
          <label v-bind:for="id + '_files'" class="custom-file-label">
            <translate key="lang_btn_add_files_to_playlist"
              >Add Files to Playlist</translate
            >
          </label>
        </div>
      </div>
    ...
    play(options) {
      ...
      return this.getStream().createFileSource(this.file, this, (source) => {
        ...
        if (this.source.duration != null) {
          ...
          if (
            ((ref1 = this.file.audio) != null ? ref1.length : void 0) != null
          ) {
